...
int main(int argc, char** argv)
{
    glfwInit();
    {
        ...
        Window window("Test", 800, 600, nullptr);
        window.setKeyCallback(key_callback);
        window.setResizeCallback(resize_callback);
        ...
        std::cout << "GL version = " << glfwGetVersionString() << std::endl;

        ShaderProgram shader_program = ShaderProgram::FromShaderFileList(
        {
        ...
        while (window.isOpen())
        {
            ...
            // drawing
            {
                shader_program.setMat4("uMVP", proj_view * backg.transform); backg.draw();
                shader_program.setMat4("uMVP", proj_view * ground.transform); ground.draw();
                shader_program.setMat4("uMVP", proj_view * bird.model.transform); bird.model.draw();

                for (const auto& tube : tubes)
                {
                    shader_program.setMat4("uMVP", proj_view * tube.first.transform); tube.first.draw();
                    shader_program.setMat4("uMVP", proj_view * tube.second.transform); tube.second.draw();
                }
            }

...
struct Scene {
  ...
  Vec3f castRay(const Vec3f& origin, const Vec3f& direction, size_t depth = 0) const {
    ...
    if (depth < 4 && (intersec_object = getIntersection(origin, direction, &point, &norm_v))) {
      ...
      for (const LightPtr light : lights) {
        ...
        if (getIntersection(shadow_origin, light_direction, nullptr, nullptr)) {
          continue;
        }
      ...
      Vec3f refraction_origin = dotProduct(refraction_direction, norm_v) < 0 ? point - norm_v * 1e-3F : point + norm_v * 1e-3F;
 ...
 private:
  ObjectPtr getIntersection(const Vec3f& origin, const Vec3f& direction, Vec3f* point, Vec3f* norm_v) const {
    ObjectPtr intersec_object = nullptr;

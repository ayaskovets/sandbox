...
TEST(column_tagged_random_access_range, enforce_concept) {
...
TEST(column_tagged_random_access_range, default_constructor) {
  mock_static_tagged_random_access_range_t();
}

TEST(column_tagged_random_access_range, constructor) {
...
TEST(column_tagged_random_access_range, range_methods_dynamic) {
...
TEST(column_tagged_random_access_range, range_methods_static) {
...
TEST(column_tagged_random_access_range, begin_end_dynamic) {
  const auto rng = mock_dynamic_tagged_random_access_range_t(
      kTag, {0, 0}, std::move(kEchoStorageProxy), 3, 4);

  EXPECT_EQ(*std::begin(rng), (index{0, 0}));
  EXPECT_EQ(*std::end(rng), (index{3, 0}));
  EXPECT_EQ(*std::rbegin(rng), (index{2, 0}));
  EXPECT_EQ(*std::rend(rng), (index{-1, 0}));
}

TEST(column_tagged_random_access_range, begin_end_static) {
  const auto rng = mock_static_tagged_random_access_range_t(
      kTag, {0, 0}, std::move(kEchoStorageProxy));

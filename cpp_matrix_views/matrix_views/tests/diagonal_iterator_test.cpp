...
TEST(diagonals_range, shape) {
  {
    const std::vector<double> data{1., 2., 3., 4., 5., 6.};
    const auto range = diagonals_range<2, 3, decltype(std::begin(data))>(
        std::begin(data), 2, 3);
  ...
  }
  {
    const std::vector<double> data{1., 2., 3., 4., 5., 6.};
    const auto range =
        diagonals_range<std::dynamic_extent, std::dynamic_extent,
...
TEST(diagonals_range, iterators) {
  {
    const std::vector<double> data{1., 2., 3., 4., 5., 6.};
    const auto range =
        diagonals_range<std::dynamic_extent, std::dynamic_extent,
    ...
    using Expected = diagonal_range<std::dynamic_extent, std::dynamic_extent,
...
TEST(diagonals_range, constness) {
  {
    std::vector<double> data{1., 2., 3., 4., 5., 6.};
    auto range =
        diagonals_range<std::dynamic_extent, std::dynamic_extent,
...
TEST(diagonals_range, correct_order) {
  {
...

    {
      ...
      EXPECT_EQ(diagonals,
                (std::vector<std::vector<int>>{{4}, {1, 5}, {2, 6}, {3}}));
...
TEST(diagonals_range, edit_range) {
  {

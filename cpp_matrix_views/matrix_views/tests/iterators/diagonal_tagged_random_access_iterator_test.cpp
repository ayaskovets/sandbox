...
TEST(diagonal_tagged_random_access_iterator, enforce_concept) {
  static_assert(
      std::random_access_iterator<mock_tagged_random_access_iterator_t>);
}

TEST(diagonal_tagged_random_access_iterator, default_constructor) {
  mock_tagged_random_access_iterator_t();
};

TEST(diagonal_tagged_random_access_iterator, constructor) {
  mock_tagged_random_access_iterator_t(kTag, kEchoIndex,
                                       std::move(kEchoStorageProxy));
};

TEST(diagonal_tagged_random_access_iterator, iterator_traits) {
  static_assert(
      std::is_same_v<index,
...
TEST(diagonal_tagged_random_access_iterator, dereference) {
  const auto it = tagged_random_access_iterator(kTag, {42, 43},
                                                std::move(kEchoStorageProxy));
  EXPECT_EQ(*it, (index{42, 43}));
  EXPECT_EQ(it->row, 42);
  EXPECT_EQ(it->column, 43);
  EXPECT_EQ(it[10], (index{52, 53}));
}

TEST(diagonal_tagged_random_access_iterator, increment_decrement) {
  auto it =
      tagged_random_access_iterator(kTag, {0, 0}, std::move(kEchoStorageProxy));

  EXPECT_EQ(*++it, (index{1, 1}));
  EXPECT_EQ(*it, (index{1, 1}));

  EXPECT_EQ(*it++, (index{1, 1}));
  EXPECT_EQ(*it, (index{2, 2}));
...
TEST(diagonal_tagged_random_access_iterator, comparison) {
  const auto it1 =
      tagged_random_access_iterator(kTag, {0, 0}, std::move(kEchoStorageProxy));
  const auto it2 =
...
TEST(diagonal_tagged_random_access_iterator, addition_subtraction) {
  const auto it =
      tagged_random_access_iterator(kTag, {5, 5}, std::move(kEchoStorageProxy));

  EXPECT_EQ(*(it + 3), (index{8, 8}));
  EXPECT_EQ(*(it - 3), (index{2, 2}));
}

TEST(diagonal_tagged_random_access_iterator, distance) {
  const auto it1 =
      tagged_random_access_iterator(kTag, {0, 0}, std::move(kEchoStorageProxy));
  const auto it2 =
      tagged_random_access_iterator(kTag, {5, 5}, std::move(kEchoStorageProxy));
...
TEST(diagonal_tagged_random_access_iterator, inplace_addition) {
  auto it =
...
TEST(diagonal_tagged_random_access_iterator, add_to_difference_type) {
  const auto it =
      tagged_random_access_iterator(kTag, {1, 1}, std::move(kEchoStorageProxy));

  EXPECT_EQ(*(2 + it), (index{3, 3}));
}
